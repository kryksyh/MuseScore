...
QAccessibleInterface* AccessibleWindowInterface::child(int index) const
{
    ...
    MYLOG() << "item: " << m_children->item()->accessibleName() << ", child: " << index << " " << (iface ? iface->text(QAccessible::Name) : "null");
    return iface;
}
...
int AccessibleWindowInterface::indexOfChild(const QAccessibleInterface* iface) const
{
    ...
    MYLOG() << "item: " << m_children->item()->accessibleName() << ", indexOfChild: " << (iface ? iface->text(QAccessible::Name) : "null") << " = " << idx;
    return idx;
}
